class EventTemplatesController < ApplicationController
  ...
  def show
    @activities = EventTemplateHasEventActivity.where(:event_id => @event_template.id)
    @aspects = EventTemplateHasAspect.where(:event_id => @event_template.id)
  end
  ...
  private
    ...
    def event_template_params
      params.require(:event_template).permit(:name, :event_type_id)
    end
end
